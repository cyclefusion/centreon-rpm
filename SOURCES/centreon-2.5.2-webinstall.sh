...
if [ ! -z $CENTENGINE ]; then
    curl -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step3.php" --data "MONITORING_ENGINE=centreon-engine&INSTALL_DIR_ENGINE=${INSTALL_DIR_ENGINE}&CENTREON_ENGINE_STATS_BINARY=${CENTREON_ENGINE_STATS_BINARY}&MONITORING_VAR_LIB=${MONITORING_VAR_LIB}&CENTREON_ENGINE_CONNECTORS=${CENTREON_ENGINE_CONNECTORS}&CENTREON_ENGINE_LIB=${CENTREON_ENGINE_LIB}&EMBEDDED_PERL=" 2> /dev/null
elif [ ! -z $NAGIOS ]; then
    curl -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step3.php" --data "MONITORING_ENGINE=nagios&INSTALL_DIR_NAGIOS=${INSTALL_DIR_NAGIOS}&NAGIOSTATS_BINARY=${NAGIOSTATS_BINARY}&NAGIOS_IMG=${NAGIOS_IMG}&EMBEDDED_PERL=${EMBEDDED_PERL}" 2> /dev/null
fi

curl -b /tmp/cicj -XGET "${URL_BASE}/step4.php" > /dev/null 2>&1
if [ ! -z $CENTBROKER ]; then
    curl -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step4.php" --data "BROKER_MODULE=centreon-broker&CENTREONBROKER_ETC=${CENTREONBROKER_ETC}&CENTREONBROKER_CBMOD=${CENTREONBROKER_CMOD}&CENTREONBROKER_LOG=${CENTREONBROKER_LOG}&CENTREONBROKER_VARLIB=${CENTREONBROKER_VARLIB}&CENTREONBROKER_LIB=${CENTREONBROKER_LIB}" 2> /dev/null
elif [ ! -z $NDO ]; then
    curl -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step4.php" --data "BROKER_MODULE=ndoutils&NDOMOD_BINARY=${NDOMOD_BINARY}" 2> /dev/null
fi
