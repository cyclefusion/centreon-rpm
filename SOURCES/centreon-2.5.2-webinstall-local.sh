...
if [ ! -z $CENTBROKER ]; then
    curl -s -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step4.php" --data "BROKER_MODULE=centreon-broker&CENTREONBROKER_ETC=${CENTREONBROKER_ETC}&CENTREONBROKER_CBMOD=${CENTREONBROKER_CBMOD}&CENTREONBROKER_LOG=${CENTREONBROKER_LOG}&CENTREONBROKER_VARLIB=${CENTREONBROKER_VARLIB}&CENTREONBROKER_LIB=${CENTREONBROKER_LIB}"
elif [ ! -z $NDO ]; then
    curl -s -b /tmp/cicj -XPOST "${URL_BASE}/process/process_step4.php" --data "BROKER_MODULE=ndoutils&NDOMOD_BINARY=${NDOMOD_BINARY}"
fi
